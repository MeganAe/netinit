...
(function seed() {
  ...
  if (count > 0) return;
  const insertLecon = db.prepare("INSERT INTO lecons (titre, resume, icone, contenu, ordre) VALUES (?, ?, ?, ?, ?)");
  const insertQuiz = db.prepare("INSERT INTO quiz (lecon_id, question, options, reponse_correcte) VALUES (?, ?, ?, ?)");
  LECONS.forEach((l, i) => {
    const res = insertLecon.run(l.titre, l.resume, l.icone, l.contenu, l.ordre);
...
module.exports = {
  ...
  getUserById: wrap((id) =>
    db.prepare("SELECT id, nom, email FROM utilisateurs WHERE id = ?").get(id)
  ),
  createUser: wrap((nom, email, hash) => {
    const res = db
      .prepare("INSERT INTO utilisateurs (nom, email, mot_de_passe) VALUES (?, ?, ?)")
      .run(nom, email, hash);
    return { id: Number(res.lastInsertRowid), nom, email };
  }),
